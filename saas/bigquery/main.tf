...
locals {
  ...
  user_output = {
    ...
    admin_project_ids      = var.admin_project_ids
  }
...
module "google_enable_api" {
  ...
  project_all  = compact(local.monitoring_projects)
...
resource "local_file" "json_file" {
  content  = jsonencode(local.user_output)
  filename = "${path.module}/output.json"

  depends_on = [module.unravel_sink, module.google_enable_admin_api, google_pubsub_topic_iam_policy.policy, module.unravel_topics, module.unravel_iam]
}
